...
describe("EthrDID", () => {
	let ethrDid, plainDid, registry, accounts, did, identity, owner, delegate1, delegate2, provider, resolver;

	beforeAll(async () => {
		// ganache-cli provider (8545)
		provider = ganache.provider();

		// // use localhost instead
		// const localhost = "http://127.0.0.1:7545";
		// // provider = new Web3.providers.HttpProvider(localhost);
		// provider = new HttpProvider(localhost); // use ethjs-provider-http instead of web3 http provider
		...
		resolver = new Resolver(getResolver({ provider, registry: registry.address }));
	});
	...
	describe("uses did for verifying aud claim", () => {
		...
		it("fails if wrong did", () => {
			return ethrDid
				...
				.then((jwt) => plainDid.verifyJWT(jwt, resolver))
				.catch((error) =>
					expect(error.message).toEqual(`JWT audience does not match your DID: aud: ${ethrDid.did} !== yours: ${plainDid.did}`)
				);
		});
	});
});
